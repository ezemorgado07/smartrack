...
// ============================================================
//  SmartRACK — api_telemetry_aht10.php
//  Recibe lecturas del sensor AHT10 desde el ESP32.
//  Temperatura y humedad interna del rack.
...
$temperature_c = round((float) $data['temperature_c'], 2);
$humidity_pct  = round((float) $data['humidity_pct'],  2);
...
if ($temperature_c < -40 || $temperature_c > 125) {
    http_response_code(400);
    echo json_encode(['success' => false, 'error' => 'temperature_c fuera de rango (-40 a 125).']);
    exit();
}
if ($humidity_pct < 0 || $humidity_pct > 100) {
    http_response_code(400);
    echo json_encode(['success' => false, 'error' => 'humidity_pct fuera de rango (0 a 100).']);
    exit();
}
