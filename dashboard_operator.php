...
<head>
    ...
    <title>SmartRACK — Operador</title>
...
<div class="wrapper">
    ...
    <div id="sr-navbar">
        ...
        <a href="dashboard_operator.php" id="sr-brand">
        ...
        <div class="sr-sidebar-inner">
            <div class="d-flex align-items-center gap-2 mb-3 mt-2 px-3">
                ...
                <div>
                    ...
                    <div class="text-white-50" style="font-size:11px;">Operador</div>
                </div>
            </div>
            <nav>
                <a class="sr-nav-link active" href="dashboard_operator.php">
    ...
    <div id="sr-content">
        ...
        <div class="sr-page-header">
            ...
            <div class="d-flex align-items-center gap-2">
                <i class="fas fa-plug" style="color:var(--at-orange);font-size:1.3rem;"></i>
                <h1 class="dash-main-title mb-0">Panel de Control</h1>
                <span class="badge ms-2" style="background:var(--at-celeste);color:#fff;font-family:'Montserrat',sans-serif;">Operador</span>
            </div>
...
document.querySelectorAll('.outlet-switch').forEach(function(chk){
    chk.addEventListener('change',function(){
        const outletNum=this.dataset.outlet,newState=this.checked?1:0,self=this;
        const pduActual = window.srPduActual || '';
        const csrfToken = document.querySelector('meta[name="csrf-token"]').getAttribute('content');
        fetch('toggle_outlet.php',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},
            body:'outlet_number='+outletNum+'&new_state='+newState
                +'&csrf_token='+encodeURIComponent(csrfToken)
                +(pduActual?'&codigo_pdu='+encodeURIComponent(pduActual):'')})
        .then(r=>r.json()).then(function(data){
            if(data.success){
                const ind=document.getElementById('dot-toma-'+outletNum),st=document.getElementById('status-text-'+outletNum);
    ...
    fetch(url).then(r=>r.json()).then(function(d){
        ...
        if(d.outlets){
            ...
            d.outlets.forEach(function(o){
                ...
                if(!dot)return;
                if(o.is_on){dot.classList.remove('status-gray');dot.classList.add('status-green');if(text){text.textContent='● ON';text.className='fw-bold ms-1 outlet-status-on';}if(chk)chk.checked=true;on++;}
